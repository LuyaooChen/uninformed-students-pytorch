...
Christian Bailer, Tewodros A Habtegebrial, Kiran
Varanasi, and Didier Stricker. Fast Dense Feature
Extraction with CNNs that have Pooling or Strid-
ing Layers. In British Machine Vision Conference
(BMVC), 2017.

Reference from: https://github.com/erezposner/Fast_Dense_Feature_Extraction
"""
...
class unwrapPool(nn.Module):
    ...
    def forward(self, x):
        y = x.view((self.outChans, self.curImgW,
                    self.curImgH, self.dH, self.dW, -1))
        y = y.transpose(2, 3)
        ...
        for i in range(0, dH):
            for j in range(0, dW):
                ...
                layers.append(nn.MaxPool2d(
                    kernel_size=(kW, kH), stride=(dW, dH)))
    ...
    def __init__(self, nInputPlane, nOutputPlane, kW, kH, dW, dH):
        ...
        for i in range(0, dH):
            for j in range(0, dW):
                ...
                a = nn.Conv2d(nInputPlane, nOutputPlane, kernel_size=(
                    kW, kH), stride=(dW, dH), padding=0)
                layers.append(a)
        self.max_layers = nn.ModuleList(layers)
        self.s = dW
